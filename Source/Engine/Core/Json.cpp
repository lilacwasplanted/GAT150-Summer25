...
namespace viper::json
{
  ...
  bool json::Read(const value_t& value, const string& name, float& data)
  {
    // check if the value has the "<name>" and the correct data type
  ...
  bool json::Read(const value_t& value, const string& name, vec3& data, bool required)
  {
    // check if the value has the "<name>" and is an array with 3 elements
    if (required) if (!value.HasMember(name.c_str()) || !value[name.c_str()].IsArray() || value[name.c_str()].Size() != 3) {
      Logger::Error("Could not read Json value (vec2): {}.", name);
      return false;
    }
